...
int TCPAcceptor::start()
{
    ...
    if (result != 0) {
        sprintf(c_err,"ERROR: bind() <id:%d>failed\n",m_id);
        err = c_err;
        LM->append(err);
        return result;
    }
    ...
    if (result != 0) {
        sprintf(c_err,"ERROR: listen() <id:%d> failed\n",m_id);
        err = c_err;
        LM->append(err);
        return result;
    }
...
TCPStream* TCPAcceptor::accept() 
{
    ...
    if (m_listening == false) {
        sprintf(c_err,"ERROR: accept() <id:%d> not listening\n",m_id);
        err = c_err;
        LM->append(err);
        return NULL;
    }
    ...
    if (sd < 0) {
        sprintf(c_err,"ERROR: accept() <id:%d> failed\n",m_id);
        err = c_err;
        LM->append(err);
        return NULL;
    }
